...
@WebServlet("/session41")
public class SessionTest41 extends HttpServlet {
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.isNew()) {
			if(user_id!=null && user_id.length()!=0) {
				...
				String url = response.encodeURL("session41");
				out.print("<a href='" + url + "'>로그인 상태 확인</a>"); // a태그는 get으로 보낸다
			}else {
